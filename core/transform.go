...
func startTransform(ctx context.Context, transform sdk.Transformer, feed <-chan sdk.Producer, feedErrs <-chan error, parallel int, report func(error)) *transformStage {
	...
	go func() {
		defer s.wg.Done()
		defer close(in)
		for msg, err := range produce(ctx, feed, feedErrs, parallel) {
			if err != nil {
